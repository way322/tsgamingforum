...
export const AntiCheat: React.FC = () => {
  const topics: Topic[] = useSelector((state: any) => state.antiCheat.topics || []); 
  ...
  return (
    <div className={s.buildsContainer}>
      <header className={s.header}>
        ...
        <div className={s.conteinerlogo}>
          <Link to="/create-topic" state={{ from: '/cs2/antiCheat' }} className={s.textizmen}>Создать тему</Link>
        ...
        {topics.length > 0 ? (
          topics.map((topic: Topic, index: number) => (
            <div className={s.topic} key={index}>
              <Link to={`/details/antiCheat/${index}`} className={s.a}>
                <div>
                ...
                <div className={s.contenerd}>
                  ...
                  <h2 className={s.text}>античит</h2>
                </div>
              </Link>
            </div>
          ))
