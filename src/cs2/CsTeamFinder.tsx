...
export const CsTeamFinder: React.FC = () => {
  const topics: Topic[] = useSelector((state: any) => state.csTeamFinder.topics || []); 
  ...
  return (
    <div className={s.buildsContainer}>
      <header className={s.header}>
        ...
        <div className={s.conteinerlogo}>
          <Link to="/create-topic" state={{ from: '/cs2/csteam-finder' }} className={s.textizmen}>Создать тему</Link>
        ...
        {topics.length > 0 ? (
          topics.map((topic: Topic, index: number) => (
            <div className={s.topic} key={index}>
              <Link to={`/details/csteam-finder/${index}`} className={s.a}>
                <div>
                ...
                <div className={s.contenerd}>
                  ...
                  <h2 className={s.text}>Поиск тимейтов</h2>
                </div>
              </Link>
            </div>
          ))
