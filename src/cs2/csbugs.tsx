...
export const Csbugs: React.FC = () => {
  const topics: Topic[] = useSelector((state: any) => state.csBugs.topics || []); 
  ...
  return (
    <div className={s.buildsContainer}>
      <header className={s.header}>
        ...
        <div className={s.conteinerlogo}>
          <Link to="/create-topic" state={{ from: '/cs2/csbugs' }} className={s.textizmen}>Создать тему</Link>
        ...
        {topics.length > 0 ? (
          topics.map((topic: Topic, index: number) => (
            <div className={s.topic} key={index}>
              <Link to={`/details/csbugs/${index}`} className={s.a}>
                <div>
