...
export const Dotabugs: React.FC = () => {
  const topics: Topic[] = useSelector((state: any) => state.bugs.topics || []);
  ...
  return (
    <div className={s.buildsContainer}>
      <header className={s.header}>
        ...
        <div className={s.conteinerlogo}>
          <Link to="/create-topic" state={{ from: '/dota2/bugs' }} className={s.textizmen}>Создать тему</Link>
        ...
        {topics.length > 0 ? (
          topics.map((topic: Topic, index: number) => (
            <div className={s.topic} key={index}>
              <Link to={`/details/bugs/${index}`} className={s.a}>
                <div>
