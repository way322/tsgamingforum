...
export const DotaMeta: React.FC = () => {
  const topics: Topic[] = useSelector((state: any) => state.meta.topics || []);
  ...
  return (
    <div className={s.buildsContainer}>
      <header className={s.header}>
        ...
        <div className={s.conteinerlogo}>
          <Link to="/create-topic" state={{ from: '/dota2/meta' }} className={s.textizmen}>Создать тему</Link>
        ...
        {topics.length > 0 ? (
          topics.map((topic: Topic, index: number) => (
            <div className={s.topic} key={index}>
              <Link to={`/details/meta/${index}`} className={s.a}>
                <div>
